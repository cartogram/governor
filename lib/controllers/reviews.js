...
exports.all = function(req, res) {
	var creative = req.query.creative || '';
	var query;
	if (creative) {
		query = Review.find({creative:creative});
	} else {
		query = Review.find();
	}
	query.sort('-created').populate('user', 'name username email').populate('creative', 'name').exec(function(err, reviews) {
		if (err) {
			res.render('error', {
				status: 500
			});
